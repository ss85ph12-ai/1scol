...
function openAddStudentModal() { 
    ...
    ['std-class','std-section','std-reg','std-name','std-phone','std-fee','std-reg-code','std-reg-name','std-driver-name','std-notes'].forEach(id=>document.getElementById(id).value=''); 
...
function submitPromote() {
    ...
    if (!newClass || !newSec || newFee <= 0) {
        return customAlert("يرجى تحديد الصف والشعبة والقسط الجديد", "error");
    }

    customConfirm("هل أنت متأكد من ترحيل الطالب للعام الجديد؟ (سيتم تصفير الدفعات والدرجات للعام الجديد لتصبح الذمة كاملة)", r => {
        if (r) {
            s.classId = newClass;
            s.sectionId = newSec;
            s.tuition = newFee;
            ...
            renderStudents();
            renderDual();
            if(typeof Swal !== 'undefined') Swal.fire({toast:true, position:'top-end', icon:'success', title:'تم ترحيل الطالب بنجاح', showConfirmButton:false, timer:1500});
        }
    });
}
...
function printReceipt() {
    ...
    if (s.siblings && s.siblings.length > 0) {
        siblingsHtml = s.siblings.map(sib => {
            ...
            return `<tr><td style="padding:10px; border-top: 1px solid #ecf0f1; border-left: 1px solid #ecf0f1; color:#e74c3c;">أخ/أخت</td><td style="padding:10px; border-top: 1px solid #ecf0f1; border-left: 1px solid #ecf0f1;">${sib.name}</td><td style="padding:10px; border-top: 1px solid #ecf0f1; border-left: 1px solid #ecf0f1;">${sib.regId || '-'}</td><td style="padding:10px; border-top: 1px solid #ecf0f1; border-left: 1px solid #ecf0f1;">${xC ? xC.name : ''}</td><td style="padding:10px; border-top: 1px solid #ecf0f1;">${xS ? xS.name : ''}</td></tr>`;
        }).join('');
    }

    let generateHalf = (title) => `
    <div class="receipt-wrapper" style="font-family:'Tajawal', Arial; padding:15px; width:95%; margin:auto; direction:rtl; box-sizing:border-box; background: #ffffff; border-radius: 20px; box-shadow: 0 5px 15px rgba(0,0,0,0.1); border: 1px solid #e0e0e0;">
        
        <div style="text-align:center; margin-bottom:15px;">
            <span style="font-size:16px; font-weight:bold; background: linear-gradient(135deg, #6e8efb, #a777e3); color: white; padding: 6px 25px; border-radius: 20px; display: inline-block; box-shadow: 0 4px 6px rgba(0,0,0,0.1); -webkit-print-color-adjust:exact;">${title}</span>
        </div>
        
        <div style="display:flex; justify-content:space-between; align-items: center; border-bottom:2px dashed #a777e3; padding-bottom:15px; margin-bottom:15px;">
            <div style="background: #f8f9fa; padding: 10px 15px; border-radius: 12px; box-shadow: inset 1px 1px 3px rgba(0,0,0,0.05); border: 1px solid #eee;"><b>التاريخ:</b><br>${new Date().toLocaleDateString('en-GB')}</div>
            <h2 style="margin:0; color:#2c3e50; text-shadow: 1px 1px 2px rgba(0,0,0,0.1); font-size:22px;">${db.schoolName}</h2>
            <div style="background: #f8f9fa; padding: 10px 15px; border-radius: 12px; box-shadow: inset 1px 1px 3px rgba(0,0,0,0.05); border: 1px solid #eee;"><b>رقم القيد:</b><br>${s.regId || '----'}</div>
        </div>

        <table style="width:100%; border-collapse:separate; border-spacing:0; text-align:center; font-weight:bold; border: 1px solid #bdc3c7; border-radius:12px; overflow:hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05); margin-bottom:15px;">
            <tr style="background: linear-gradient(135deg, #e0c3fc, #8ec5fc); color: #2c3e50; -webkit-print-color-adjust:exact;">
                <td style="padding:10px; border-bottom: 1px solid #bdc3c7; border-left: 1px solid #bdc3c7;">علاقة</td><td style="padding:10px; border-bottom: 1px solid #bdc3c7; border-left: 1px solid #bdc3c7;">الاسم</td><td style="padding:10px; border-bottom: 1px solid #bdc3c7; border-left: 1px solid #bdc3c7;">القيد</td><td style="padding:10px; border-bottom: 1px solid #bdc3c7; border-left: 1px solid #bdc3c7;">الصف</td><td style="padding:10px; border-bottom: 1px solid #bdc3c7;">الشعبة</td>
            </tr>
            <tr style="background:#fff;">
                <td style="padding:10px; border-left: 1px solid #ecf0f1;">الرئيسي</td><td style="padding:10px; border-left: 1px solid #ecf0f1;">${s.name}</td><td style="padding:10px; border-left: 1px solid #ecf0f1;">${s.regId || '-'}</td><td style="padding:10px; border-left: 1px solid #ecf0f1;">${c ? c.name : ''}</td><td style="padding:10px;">${sc ? sc.name : ''}</td>
            </tr>
            ${siblingsHtml}
        </table>

        <table style="width:100%; border-collapse:separate; border-spacing:0; text-align:center; font-weight:bold; border: 1px solid #bdc3c7; border-radius:12px; overflow:hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05); margin-bottom:15px;">
            <tr style="background: linear-gradient(135deg, #fdfbfb, #ebedee); color: #2c3e50; -webkit-print-color-adjust:exact;">
                <td style="padding:10px; border-bottom: 1px solid #bdc3c7; border-left: 1px solid #bdc3c7;">المبلغ الكلي</td><td style="padding:10px; border-bottom: 1px solid #bdc3c7; border-left: 1px solid #bdc3c7;">الواصل (المسدد)</td><td colspan="2" style="padding:10px; border-bottom: 1px solid #bdc3c7;">المتبقي (الذمة)</td>
            </tr>
            <tr style="background:#fff;">
                <td style="padding:10px; font-size:16px; border-left: 1px solid #ecf0f1; color:#2980b9;">${totTuition.toLocaleString()}</td><td style="padding:10px; font-size:16px; border-left: 1px solid #ecf0f1; color:#27ae60;">${p.toLocaleString()}</td><td colspan="2" style="padding:10px; font-size:16px; color:#c0392b; background: rgba(231, 76, 60, 0.05); -webkit-print-color-adjust:exact;">${(totTuition - p).toLocaleString()}</td>
            </tr>
        </table>

        <table style="width:100%; border-collapse:separate; border-spacing:0; text-align:center; font-size: 13px; font-weight:bold; border: 1px solid #bdc3c7; border-radius:12px; overflow:hidden; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
            <tr style="background: linear-gradient(135deg, #e0c3fc, #8ec5fc); color: #2c3e50; -webkit-print-color-adjust:exact;">
                <td colspan="2" style="border-bottom: 1px solid #bdc3c7; border-left: 1px solid #bdc3c7; padding:5px;">الدفعة 1</td>
                <td colspan="2" style="border-bottom: 1px solid #bdc3c7; border-left: 1px solid #bdc3c7; padding:5px;">الدفعة 2</td>
                <td colspan="2" style="border-bottom: 1px solid #bdc3c7; border-left: 1px solid #bdc3c7; padding:5px;">الدفعة 3</td>
                <td colspan="2" style="border-bottom: 1px solid #bdc3c7; padding:5px;">الدفعة 4</td>
            </tr>
            <tr style="background:#f8f9fa; -webkit-print-color-adjust:exact;">
                <td style="border-bottom: 1px solid #bdc3c7; border-left: 1px dotted #bdc3c7; padding:4px;">المبلغ</td><td style="border-bottom: 1px solid #bdc3c7; border-left: 1px solid #bdc3c7; padding:4px;">التاريخ</td>
                <td style="border-bottom: 1px solid #bdc3c7; border-left: 1px dotted #bdc3c7; padding:4px;">المبلغ</td><td style="border-bottom: 1px solid #bdc3c7; border-left: 1px solid #bdc3c7; padding:4px;">التاريخ</td>
                <td style="border-bottom: 1px solid #bdc3c7; border-left: 1px dotted #bdc3c7; padding:4px;">المبلغ</td><td style="border-bottom: 1px solid #bdc3c7; border-left: 1px solid #bdc3c7; padding:4px;">التاريخ</td>
                <td style="border-bottom: 1px solid #bdc3c7; border-left: 1px dotted #bdc3c7; padding:4px;">المبلغ</td><td style="border-bottom: 1px solid #bdc3c7; padding:4px;">التاريخ</td>
            </tr>
            <tr style="background:#fff;">
                <td style="border-bottom: 1px solid #bdc3c7; border-left: 1px dotted #ecf0f1; padding:5px;">${s.payments[0]?.amount?.toLocaleString() || 0}</td><td style="border-bottom: 1px solid #bdc3c7; border-left: 1px solid #ecf0f1; padding:5px;">${s.payments[0]?.date || '-'}</td>
                <td style="border-bottom: 1px solid #bdc3c7; border-left: 1px dotted #ecf0f1; padding:5px;">${s.payments[1]?.amount?.toLocaleString() || 0}</td><td style="border-bottom: 1px solid #bdc3c7; border-left: 1px solid #ecf0f1; padding:5px;">${s.payments[1]?.date || '-'}</td>
                <td style="border-bottom: 1px solid #bdc3c7; border-left: 1px dotted #ecf0f1; padding:5px;">${s.payments[2]?.amount?.toLocaleString() || 0}</td><td style="border-bottom: 1px solid #bdc3c7; border-left: 1px solid #ecf0f1; padding:5px;">${s.payments[2]?.date || '-'}</td>
                <td style="border-bottom: 1px solid #bdc3c7; border-left: 1px dotted #ecf0f1; padding:5px;">${s.payments[3]?.amount?.toLocaleString() || 0}</td><td style="border-bottom: 1px solid #bdc3c7; padding:5px;">${s.payments[3]?.date || '-'}</td>
            </tr>
            <tr style="background: linear-gradient(135deg, #e0c3fc, #8ec5fc); color: #2c3e50; -webkit-print-color-adjust:exact;">
                <td colspan="2" style="border-bottom: 1px solid #bdc3c7; border-left: 1px solid #bdc3c7; padding:5px;">الدفعة 5</td>
                <td colspan="2" style="border-bottom: 1px solid #bdc3c7; border-left: 1px solid #bdc3c7; padding:5px;">الدفعة 6</td>
                <td colspan="2" style="border-bottom: 1px solid #bdc3c7; border-left: 1px solid #bdc3c7; padding:5px;">الدفعة 7</td>
                <td colspan="2" style="border-bottom: 1px solid #bdc3c7; padding:5px;">الدفعة 8</td>
            </tr>
            <tr style="background:#fff;">
                <td style="border-left: 1px dotted #ecf0f1; padding:5px;">${s.payments[4]?.amount?.toLocaleString() || 0}</td><td style="border-left: 1px solid #ecf0f1; padding:5px;">${s.payments[4]?.date || '-'}</td>
                <td style="border-left: 1px dotted #ecf0f1; padding:5px;">${s.payments[5]?.amount?.toLocaleString() || 0}</td><td style="border-left: 1px solid #ecf0f1; padding:5px;">${s.payments[5]?.date || '-'}</td>
                <td style="border-left: 1px dotted #ecf0f1; padding:5px;">${s.payments[6]?.amount?.toLocaleString() || 0}</td><td style="border-left: 1px solid #ecf0f1; padding:5px;">${s.payments[6]?.date || '-'}</td>
                <td style="border-left: 1px dotted #ecf0f1; padding:5px;">${s.payments[7]?.amount?.toLocaleString() || 0}</td><td style="padding:5px;">${s.payments[7]?.date || '-'}</td>
            </tr>
        </table>
        
        <div style="font-size:12px; text-align:center; margin-top:15px; border-top:1px dashed #ccc; padding-top:10px; color: #7f8c8d;">ولي الأمر ملزم بدفع المبلغ كاملاً دون قطع أي مبلغ في حال النقل أو الترك</div>
    </div>`;

    let html = `
    <div style="display:flex; flex-direction:column; height:100vh; justify-content:space-around;">
        ${generateHalf('نسخة المدرسة')}
        <div class="receipt-divider" style="border-top:2px dashed #a777e3; width:95%; margin:0 auto;"></div>
...
function exportStudentExcel() { 
    try {
        ...
        let totTuition = s.tuition + (s.siblings||[]).reduce((a,b)=>a+(b.fee||0),0);
        let data=[["العلاقة","الاسم","القيد","الموبايل","ملاحظات","الصف","الشعبة","الكلي","الواصل","المتبقي"],["الرئيسي",s.name,s.regId||'',s.phone||'',s.notes||'',c?c.name:'',sc?sc.name:'',totTuition,p,totTuition-p]]; 
