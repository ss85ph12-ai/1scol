...
function showRequiredFieldsAlert(missingLabels) {
    playValidationSound();
    if(typeof Swal !== 'undefined') {
        Swal.fire({
            icon: 'warning',
            title: 'يوجد حقل فارغ',
            html: `<div class="required-alert-text">يرجى إكمال الحقول المطلوبة قبل الحفظ:<br><b>${missingLabels.join('، ')}</b></div>`,
            confirmButtonText: 'حسنًا',
            showCloseButton: true,
            customClass: { popup: 'swal2-glass required-alert-popup' },
            showClass: { popup: 'validation-pop-in' },
            hideClass: { popup: 'validation-pop-out' }
        });
    } else {
        alert('يوجد حقل فارغ: ' + missingLabels.join('، '));
    }
}
function markRequiredField(id, invalid) {
    const el = document.getElementById(id);
    if(!el) return;
    el.classList.toggle('field-required-missing', invalid);
    if(invalid) {
        el.addEventListener('input', () => el.classList.remove('field-required-missing'), { once: true });
        el.addEventListener('change', () => el.classList.remove('field-required-missing'), { once: true });
    }
}
...
function getStudentRegistrationDate(s) {
    if(s.registrationDate) return s.registrationDate;
    if(s.startDate) return s.startDate;
    if(typeof s.id === 'number' && s.id > 1000000000000) return formatISODate(new Date(s.id)) || '-';
    return '-';
}
...
function saveStudent() {
    const requiredFields = [
        { id: 'std-class', label: 'الصف', isValid: v => v !== '' },
        { id: 'std-section', label: 'الشعبة', isValid: v => v !== '' },
        { id: 'std-reg', label: 'رقم القيد', isValid: v => v !== '' },
        { id: 'std-name', label: 'الاسم الثلاثي للطالب', isValid: v => v !== '' },
        { id: 'std-phone', label: 'رقم الموبايل (10 أرقام)', isValid: v => /^\d{10}$/.test(v) },
        { id: 'std-fee', label: 'القسط الكلي المقرر (أكبر من صفر)', isValid: v => (parseFloat(v) || 0) > 0 }
    ];
    let missing = requiredFields.filter(f => {
        let value = String(document.getElementById(f.id).value || '').trim();
        return !f.isValid(value);
    });
    requiredFields.forEach(f => markRequiredField(f.id, missing.some(m => m.id === f.id)));
    if(missing.length) {
        let first = document.getElementById(missing[0].id);
        showRequiredFieldsAlert(missing.map(f => f.label));
    ...
    if(editingStudentId){ let idx=db.students.findIndex(x=>x.id===editingStudentId); d.id=db.students[idx].id; d.payments=db.students[idx].payments; d.grades=db.students[idx].grades; d.registrationDate=db.students[idx].registrationDate || getStudentRegistrationDate(db.students[idx]); db.students[idx]=d; if(typeof Swal !== 'undefined') Swal.fire({toast:true,position:'top-end',icon:'success',title:'تم التعديل',showConfirmButton:false,timer:1500}); } 
    else { d.id=Date.now(); d.registrationDate=todayISO; d.payments=[]; d.grades={}; db.students.push(d); if(typeof Swal !== 'undefined') Swal.fire({toast:true,position:'top-end',icon:'success',title:'تم الإضافة',showConfirmButton:false,timer:1500}); }
...
function openProfile(id) {
    ...
    document.getElementById('prof-details').innerText += ` | تاريخ التسجيل: ${getStudentRegistrationDate(s)}`;
...
function updateFinance(s) {
    ...
    let paid=s.payments.reduce((sum,p)=>sum+(parseFloat(p.amount)||0),0);
    let paymentCount = s.payments.length;
    document.getElementById('prof-total').innerText=totTuition.toLocaleString();
    document.getElementById('prof-paid').innerText=paid.toLocaleString();
    document.getElementById('prof-rem').innerText=(totTuition-paid).toLocaleString();
    document.getElementById('prof-pay-count').innerText=paymentCount.toLocaleString();
    let summaryHtml = `<div class="list-item payment-summary p-2">
        <div style="font-weight:bold;"><i class="fas fa-receipt"></i> كشف الأقساط: ${paymentCount} دفعة</div>
        <small>مجموع المدفوع: ${paid.toLocaleString()} د.ع | المتبقي: ${(totTuition-paid).toLocaleString()} د.ع</small>
    </div>`;
    let rowsHtml = s.payments.map((p,i)=>{
        let amount = parseFloat(p.amount)||0;
        return `<div class="list-item payment-row flex-between p-2">
            <span><b>دفعة ${i+1}:</b> ${amount.toLocaleString()} د.ع | ${p.date || '-'} ${p.receiptNo ? `<br><small style="color:#a777e3">(رقم الوصل: ${p.receiptNo})</small>` : ''}</span>
            <span class="payment-actions">
                <button type="button" class="btn-3d warning btn-small m-0" title="تعديل الدفعة" onclick="openPaymentModal(${i})"><i class="fas fa-edit"></i> تعديل</button>
                <button type="button" class="btn-3d danger btn-small m-0" title="حذف الدفعة" onclick="delPayment(${i})"><i class="fas fa-trash"></i> حذف</button>
            </span>
        </div>`;
    }).join('');
    document.getElementById('payments-history').innerHTML = summaryHtml + (rowsHtml || '<div class="list-item p-2 text-center">لا توجد دفعات مستلمة بعد</div>');
}
function openPaymentModal(i = null) {
    let s = db.students.find(x=>x.id===currentStudentId);
    editingPaymentIndex = Number.isInteger(i) ? i : null;
    let isEditing = editingPaymentIndex !== null && s && s.payments && s.payments[editingPaymentIndex];
    let modal = document.getElementById('payment-modal');
    let title = modal.querySelector('h3');
    let button = modal.querySelector('button[onclick="submitPayment()"]');
    document.getElementById('pay-date').value = isEditing ? (s.payments[editingPaymentIndex].date || todayISO) : todayISO;
    document.getElementById('pay-amount').value = isEditing ? (parseFloat(s.payments[editingPaymentIndex].amount)||'') : '';
    title.innerText = isEditing ? 'تعديل دفعة الطالب' : 'استلام قسط من الطالب';
    button.innerHTML = isEditing ? '<i class="fas fa-save"></i> حفظ تعديل الدفعة' : '<i class="fas fa-print"></i> حفظ وطباعة الوصل';
...
function submitPayment() { 
    ...
    if(amt>0 && dt){ 
        let s=db.students.find(x=>x.id===currentStudentId); 
        if(!Array.isArray(s.payments)) s.payments = [];
        let isEditing = editingPaymentIndex !== null && s.payments[editingPaymentIndex];
        if(isEditing) {
            s.payments[editingPaymentIndex] = {...s.payments[editingPaymentIndex], amount:amt, date:dt};
        } else {
            if(!db.settings.receiptCounter) db.settings.receiptCounter = 1000;
            let currentRecNo = db.settings.receiptCounter++;
            s.payments.push({amount:amt, date:dt, receiptNo: currentRecNo}); 
        }
        saveDB(); document.getElementById('pay-amount').value=''; updateFinance(s); renderStudents(); renderDual(); renderDaily(); 
        hideModal('payment-modal');
        if(typeof Swal !== 'undefined') Swal.fire({toast:true, position:'top-end', icon:'success', title:isEditing ? 'تم تعديل الدفعة' : 'تم تسديد الدفعة', showConfirmButton:false, timer:1500}); 
        if(!isEditing) setTimeout(() => printReceipt(), 500);
        editingPaymentIndex = null;
    } else { customAlert("يرجى إدخال المبلغ والتاريخ بشكل صحيح", "error"); }
}
function delPayment(i) { customConfirm('حذف الدفعة المحددة؟', r=>{ if(r){let s=db.students.find(x=>x.id===currentStudentId); s.payments.splice(i,1); editingPaymentIndex = null; saveDB(); updateFinance(s); renderStudents(); renderDual(); renderDaily();} }); }
...
// ============ 7. الرواتب والموظفين ============
function getYearFromDate(dateValue, fallbackYear = new Date().getFullYear()) {
    if(dateValue) {
...
function getServiceYears(st) {
    let startYear = getYearFromDate(st.startDate, st.hiringYear || new Date().getFullYear());
    return Math.max(0, new Date().getFullYear() - startYear);
}
function getCurrentMonthKey() {
    return todayISO.slice(0, 7);
}
function paymentMatchesMonth(dateValue, monthKey) {
    if(!dateValue || !monthKey) return false;
    let raw = String(dateValue).trim();
    if(raw.startsWith(monthKey)) return true;
    let parts = raw.includes('/') ? raw.split('/') : raw.split('-');
    if(parts.length === 3 && parts[2].length === 4) {
        let normalized = `${parts[2]}-${String(parts[1]).padStart(2, '0')}`;
...
function calcStaffSalary() {
    if(currentStaffPaymentId === null) return 0;
    ...
    let students = parseFloat(document.getElementById('staff-pay-students').value) || 0;
    let serviceAmount = parseFloat(document.getElementById('staff-pay-service-amount').value) || 0;
    let bonus = parseFloat(document.getElementById('staff-pay-bonus').value) || 0;
    let deduct = parseFloat(document.getElementById('staff-pay-deduct').value) || 0;

    let serviceBonus = serviceYears * serviceAmount;
    let net = nominalSalary + students + serviceBonus + bonus - deduct;

    document.getElementById('staff-pay-service-total').innerText = serviceBonus.toLocaleString();
    document.getElementById('staff-pay-net').innerText = net.toLocaleString();
    return net;
}

function submitStaffPayment() {
    let net = calcStaffSalary();
    let dt = document.getElementById('staff-pay-date').value;
    let st = db.staff.find(x => x.id === currentStaffPaymentId);
    if(net > 0 && dt && st) {
...
function renderStaffPaymentHistory(st) {
    if(!Array.isArray(st.payments) || st.payments.length === 0) {
        return '<div class="mt-1"><small style="opacity:0.75;"><i class="fas fa-info-circle"></i> لا توجد دفعات مسجلة لهذا الموظف</small></div>';
    }
    ...
    return `<details class="glass mt-1" style="padding:8px 10px;">
        <summary style="cursor:pointer; font-weight:bold;"><i class="fas fa-list-alt"></i> كشف الدفعات السابقة (${st.payments.length})</summary>
...
function renderStaffMonthlyReport() {
    let monthInput = document.getElementById('staff-pay-month');
    let reportEl = document.getElementById('staff-month-report');
    if(!monthInput || !reportEl) return;
    if(!monthInput.value) monthInput.value = getCurrentMonthKey();
    let monthKey = monthInput.value;
    let totalMonth = 0;
    let rows = db.staff.map(st => {
        if(!Array.isArray(st.payments)) st.payments = [];
        let monthPayments = st.payments.filter(p => paymentMatchesMonth(p.date, monthKey));
        let total = monthPayments.reduce((sum,p)=>sum+(parseFloat(p.amount)||0),0);
        totalMonth += total;
        let details = monthPayments.length
            ? monthPayments.map(p => `<span class="staff-payment-chip">${p.date || '-'}: ${(parseFloat(p.amount)||0).toLocaleString()} د.ع</span>`).join('')
            : '<small style="opacity:.75;">لا توجد دفعات بهذا الشهر</small>';
        return `<div class="list-item staff-month-row p-2">
            <div class="flex-between">
                <span><strong>${st.name}</strong> <small>(${st.role || (st.isTeacher ? 'مدرس' : 'إداري')})</small></span>
                <b>${total.toLocaleString()} د.ع</b>
            </div>
            <div class="staff-payment-dates mt-1">${details}</div>
        </div>`;
    }).join('');
    reportEl.innerHTML = `<div class="list-item staff-month-total p-2 flex-between">
        <span><i class="fas fa-calendar-alt"></i> مجموع رواتب الكادر لهذا الشهر</span>
        <b>${totalMonth.toLocaleString()} د.ع</b>
    </div>` + (rows || '<div class="text-center mt-1">لا توجد بيانات كادر</div>');
}
...
function printReceipt() {
    ...
    let totTuition = s.tuition + (s.siblings||[]).reduce((a,b)=>a+(b.fee||0),0);
    let p = s.payments.reduce((a, b) => a + (parseFloat(b.amount)||0), 0);
    ...
    <div class="receipt-print-container" style="border: 2px solid #000; padding: 10px; font-family: Arial, sans-serif; direction: rtl; width: 100%; box-sizing: border-box; height: auto; display: flex; flex-direction: column; justify-content: space-between; margin: 0 auto; break-inside: avoid; page-break-inside: avoid;">
    ...
    let html = `
    <div style="width:100%; box-sizing: border-box; background: #fff; display: flex; flex-direction: column; gap: 3mm;">
        <div style="height: auto; padding-top: 0; break-inside: avoid; page-break-inside: avoid;">
